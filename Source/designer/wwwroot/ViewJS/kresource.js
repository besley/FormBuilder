...
var kresource = (function () {
    ...
    kresource.setLang = function (lang) {
        ...
        if (typeof mxClient !== 'undefined') mxClient.language = lang;
        window.localStorage.setItem(kresource.MX_RESOURCE_LOCAL_LANGUAGE, lang);
    }

    kresource.localize = function (isActionIncluded) {
        var path = 'Resources/';
        if (isActionIncluded !== null && isActionIncluded === true) {
            path = '../../' + path;
        }

        if (kresource.mxCurrentLanguageJSON === null) {
            var lang = kresource.getLang();
            jshelper.ajaxGet(path + lang + ".json", null,
                function (json) {
                    kresource.mxCurrentLanguageJSON = json;
                    renderLang(json);
                });
        } else {
            renderLang(kresource.mxCurrentLanguageJSON);
        }
    }

    function renderLang(json) {
    ...
    kresource.getItem = function (key, message) {
        var json = kresource.mxCurrentLanguageJSON;
        var text = json[key];

        if (message !== undefined) text = text + message;
        return text;
    }    
    return kresource;
})()