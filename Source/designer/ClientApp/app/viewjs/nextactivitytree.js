import kprocessrun from "./kprocessrun";
import { getSchemaVariables } from '@bpmn-io/form-js';


const nextActivityTree = (function () {
    ...
    nextActivityTree.getNextActivityTree = function (runner) {
        ...
        jshelper.ajaxPost(kconfig.webApiUrl + 'api/wf2xml/GetNextStepInfo',
            JSON.stringify(runner),
            function (result) {
                if (result.Status === 1) {
                    //弹窗步骤人员办理弹窗
                    var nextStep = result.Entity.NextActivityRoleUserTree[0];        //单步演示
    ...
    function fillConditionKey() {
        ...
        var variables = getSchemaVariables(kprocessrun.mxFormDataSchema);
        console.log('get form variables...')
    ...
    nextActivityTree.sure = function () {
        ...
        var taskEntity = kprocessrun.mxSelectedToDoTaskEntity;

        var runner = {};
        runner.TaskID = taskEntity.TaskID;
        runner.ProcessGUID = taskEntity.ProcessGUID;
        runner.Version = taskEntity.Version;
        runner.AppInstanceID = taskEntity.AppInstanceID;
        runner.AppName = taskEntity.AppName;
        runner.UserID = taskEntity.AssignedToUserID;
        runner.UserName = taskEntity.AssignedToUserName;

        runner.NextActivityPerformers = {};
        runner.NextActivityPerformers[activityGUID] = userlist;
        runner.Conditions = fillConditionKey();

        kmsgbox.confirm(kresource.getItem('NextStepTree.runProcess.confirm') + activityName, function () {
            jshelper.ajaxPost(kconfig.webApiUrl + "api/wf2xml/RunProcess", JSON.stringify(runner), function (result) {
                if (result.Status === 1) {
                    //refresh task list
                    kprocessrun.refreshToDoTask();
                    kprocessrun.refreshDoneTask();
...
export default nextActivityTree;
